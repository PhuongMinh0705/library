...
$sql="update  dulieu_theloai set CategoryName=:category,Status=:status where id=:catid";
...
<?php 
$catid=intval($_GET['catid']);
$sql="SELECT * from dulieu_theloai where id=:catid";
$query=$dbh->prepare($sql);
$query-> bindParam(':catid',$catid, PDO::PARAM_STR);
$query->execute();
...
<div class="radio">
<label>
<input type="radio" name="status" id="status" value="0" checked="checked">Ẩn
</label>
</div>
 <div class="radio">
<label>
<input type="radio" name="status" id="status" value="1">Kích hoạt
</label>
</div
<?php } ?>
</div>
